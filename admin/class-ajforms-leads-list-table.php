	...
	protected function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'actions':
				...
				// LEAD STATUS pipeline — combined into this same block (not a separate column) so
				// the badge and every action stay in one clickable group. Only a "Next: {stage}"
				// quick action is offered here (jumping to an arbitrary stage still requires Open,
				...
				if ( '' !== $next_stage ) {
					$next_url = wp_nonce_url(
						add_query_arg(
							array( 'page' => 'ajforms-leads', 'lead_action' => 'set_pipeline_status', 'pipeline_status' => $next_stage, 'lead_id' => $lead_id ),
							admin_url( 'admin.php' )
						),
						'ajf_lead_action_' . $lead_id
					);
					$out .= '<a class="button button-small" href="' . esc_url( $next_url ) . '">' . esc_html( sprintf( __( 'Next: %s', 'ajforms' ), $pipeline_labels[ $next_stage ] ) ) . '</a>';
				}
