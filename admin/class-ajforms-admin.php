	...
	private function handle_client_portal_settings_save() {
		...
		if ( isset( $_POST['ajcore_portal_products_nonce'] ) ) {
			...
			foreach ( $product_rows as $product_id => $row ) {
				...
				$duplicate_behavior = isset( $row['duplicate_behavior'] ) ? sanitize_key( $row['duplicate_behavior'] ) : 'hide_if_owned';
				if ( ! in_array( $duplicate_behavior, array( 'allow_duplicate', 'hide_if_owned', 'custom_request_if_owned' ), true ) ) {
					$duplicate_behavior = 'hide_if_owned';
				}
				$custom_request_status = isset( $row['custom_request_status'] ) ? sanitize_key( $row['custom_request_status'] ) : 'admin_review_required';
				if ( '' === $custom_request_status ) {
					$custom_request_status = 'admin_review_required';
				}
				...
				$wpdb->update(
					...
					array(
						...
						'custom_request_status'       => $custom_request_status,
						'custom_request_type'         => isset( $row['custom_request_type'] ) ? sanitize_key( $row['custom_request_type'] ) : 'custom_pricing_request',
					),
					array( 'id' => $product_id ),
						...
						<?php else : ?>
							<?php foreach ( $products as $product ) : ?>
								<tr>
									...
									<td>
										<?php $duplicate_behavior = isset( $product->duplicate_behavior ) && in_array( $product->duplicate_behavior, array( 'allow_duplicate', 'hide_if_owned', 'custom_request_if_owned' ), true ) ? $product->duplicate_behavior : 'hide_if_owned'; ?>
										<select style="width:100%;margin-bottom:6px" name="portal_products[<?php echo esc_attr( (int) $product->id ); ?>][duplicate_behavior]">
											<option value="hide_if_owned" <?php selected( $duplicate_behavior, 'hide_if_owned' ); ?>><?php esc_html_e( 'Do not allow duplicate', 'ajforms' ); ?></option>
											<option value="allow_duplicate" <?php selected( $duplicate_behavior, 'allow_duplicate' ); ?>><?php esc_html_e( 'Allow duplicate', 'ajforms' ); ?></option>
											<option value="custom_request_if_owned" <?php selected( $duplicate_behavior, 'custom_request_if_owned' ); ?>><?php esc_html_e( 'Show custom request if already owned', 'ajforms' ); ?></option>
										</select>
										...
										<textarea style="width:100%;min-height:60px;margin-bottom:6px" name="portal_products[<?php echo esc_attr( (int) $product->id ); ?>][custom_request_message]" placeholder="<?php esc_attr_e( 'Custom request message', 'ajforms' ); ?>"><?php echo esc_textarea( isset( $product->custom_request_message ) ? $product->custom_request_message : '' ); ?></textarea>
										<input type="text" style="width:100%;margin-bottom:6px" name="portal_products[<?php echo esc_attr( (int) $product->id ); ?>][custom_request_button_label]" value="<?php echo esc_attr( isset( $product->custom_request_button_label ) ? $product->custom_request_button_label : '' ); ?>" placeholder="<?php esc_attr_e( 'Button label, example: Request Additional Entity Pricing', 'ajforms' ); ?>">
										<input type="hidden" name="portal_products[<?php echo esc_attr( (int) $product->id ); ?>][custom_request_status]" value="admin_review_required">
										<input type="hidden" name="portal_products[<?php echo esc_attr( (int) $product->id ); ?>][custom_request_type]" value="custom_pricing_request">
									</td>
