...
class AJCore_REST_API {
	...
	private function get_current_user_file_rows() {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT f.id, f.attachment_id, f.title, f.description, f.category, f.created_at, f.updated_at FROM `{$files_table}` f INNER JOIN `{$link_table}` fu ON fu.file_id = f.id WHERE f.status = 'active' AND ( fu.user_id = %d OR fu.user_email = %s ) ORDER BY f.created_at DESC, f.id DESC LIMIT 100",
				(int) $user->ID,
				$user->user_email
			),
			ARRAY_A
