...
- **Settings** — two columns (stacked on narrow screens). The left column is Google: a **Connection status** checklist naming the three prerequisites (OAuth client saved, Google account connected, business location selected) with the identity or the next action for each, then the redirect URI, the credentials form with an inline "where do I get these" walkthrough, the OAuth/accounts/test/disconnect buttons, the **Business account and location** pickers — always shown with a line saying where the loaded lists appear and that they expire after 15 minutes — and the sync status panel. The right column is the Rate Us prompt and display options. **Load Business Accounts**, **Load Locations**, **Use This Location** and **Test Connection** each report a distinct outcome in the notice at the top of the page instead of a generic "Operation completed". The last ten sync-history rows sit in a collapsed **Recent sync activity** block; the option still keeps at most 50 entries of timestamp, safe status/error code, processed count, duration, and trigger. The UI translates the safe code into a fixed summary. API bodies, reviewer data, exception messages, and credentials are never logged there.
